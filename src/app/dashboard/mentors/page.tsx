...
export default function MentorsPage() {
  ...
  const handleAdd = async () => {
    try {
      ...
      showToast("Mentor added — fill in the details below");
    } catch (err: unknown) {
  ...
  const handlePhotoUpload = async (id: string, file: File) => {
    ...
    try {
      ...
      const url = await uploadFile(file, path, (progress) => setUploadProgress(progress));
  ...
  return (
    <div className="flex flex-col min-h-screen relative" style={{ background: "var(--cms-bg)" }}>
      <Topbar title="Ecosystem Enablers" breadcrumb="Mentors" />

      {/* Toast */}
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-xl text-white text-xs font-semibold animate-fade-in border ${
            toast.type === "success"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {mentors.map((mentor) => (
              ...
              >
                {/* Card Header — photo + name + active badge */}
                ...
                <div className="px-5 py-4 space-y-3 flex-1">
                  ...
                  <div>
                    ...
                    <textarea
                      id={`bio-${mentor.id}`}
                      className="cms-input w-full text-sm resize-none leading-relaxed"
                      rows={3}
                      value={mentor.bio ?? ""}
                ...
                <div className="px-5 py-3 border-t border-[#F5F5F5] bg-[#FAFAFA] flex items-center justify-between gap-3">
                  {/* Left: Active pill button + Order */}
                  <div className="flex items-center gap-3">
                    {/* Labeled active toggle pill */}
                    <button
                      type="button"
