...
import { Link2, Trash2, Save, Camera, Plus, CheckCircle2, XCircle, X, AlertTriangle, Users, ImagePlus } from "lucide-react";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] animate-fade-in">
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1 space-y-4">
          ...
          <div>
            ...
            <input type="text" autoFocus className={`cms-input w-full ${errors.name ? "border-red-300 bg-red-50" : ""}`}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="url" className="cms-input w-full" style={{ paddingLeft: "32px" }}
                placeholder="https://linkedin.com/in/..." value={linkedIn} onChange={(e) => setLinkedIn(e.target.value)} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div>
              ...
              <input type="number" className="cms-input w-20 text-center" value={order} onChange={(e) => setOrder(Number(e.target.value))} />
            </div>
            <div className="flex-1">
              ...
              <button type="button" onClick={() => setActive((v) => !v)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider border transition-all ${
                  active ? "bg-green-50 text-green-700 border-green-200" : "bg-[#F5F5F5] text-[#888888] border-[#E0E0E0]"
                }`}>
...
export default function SocialMentorsPage() {
  ...
  useEffect(() => {
    const unsub = subscribeSocialMentors((data) => { setMentors(data); setLoading(false); });
    return () => unsub();
  }, []);
    ...
    <div className="flex flex-col min-h-screen" style={{ background: "var(--cms-bg)" }}>
      ...
      <main className="flex-1 px-8 py-8 space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          ...
          <button id="add-social-mentor-btn" onClick={() => setShowAddModal(true)}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {mentors.map((mentor) => (
              <div key={mentor.id} id={`social-mentor-${mentor.id}`}
                ...
                <div className="px-5 pt-5 pb-4 flex items-start gap-4 border-b border-[#F5F5F5]">
                  <div className="relative flex-shrink-0">
                    ...
                    <button onClick={() => fileInputRefs.current[mentor.id]?.click()} disabled={uploadingId === mentor.id}
                      className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-[#800020] flex items-center justify-center shadow-md hover:bg-[#660019] transition-colors" title="Upload photo">
                ...
                <div className="px-5 py-4 space-y-3 flex-1">
                  <div className="grid grid-cols-[1fr_80px] gap-3">
                    <div>
                      ...
                      <input type="text" className="cms-input w-full text-sm" value={mentor.name ?? ""}
                        onChange={(e) => handleUpdate(mentor.id, "name", e.target.value)} placeholder="Full name" />
                    </div>
                    <div>
                      <label className="block text-[9px] font-extrabold uppercase tracking-[1.5px] text-[#AAAAAA] mb-1">Initials</label>
                      <input type="text" className="cms-input w-full text-sm text-center font-bold" value={mentor.initials ?? ""}
                        onChange={(e) => handleUpdate(mentor.id, "initials", e.target.value)} maxLength={3} placeholder="AB" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[9px] font-extrabold uppercase tracking-[1.5px] text-[#AAAAAA] mb-1">Role / Designation</label>
                    <input type="text" className="cms-input w-full text-sm" value={mentor.role ?? ""}
                      onChange={(e) => handleUpdate(mentor.id, "role", e.target.value)} placeholder="e.g. Social Innovator" />
                  </div>
                  <div>
                    <label className="block text-[9px] font-extrabold uppercase tracking-[1.5px] text-[#AAAAAA] mb-1">LinkedIn URL</label>
                    <div className="relative">
                      <Link2 className="w-3.5 h-3.5 text-[#AAAAAA] absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                      <input type="url" className="cms-input w-full text-sm" style={{ paddingLeft: "32px" }} value={mentor.linkedIn ?? ""}
                        onChange={(e) => handleUpdate(mentor.id, "linkedIn", e.target.value)} placeholder="https://linkedin.com/in/..." />
                    </div>
                  </div>
                </div>
                ...
                <div className="px-5 py-3 border-t border-[#F5F5F5] bg-[#FAFAFA] flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleSave(mentor)} disabled={saving === mentor.id}
